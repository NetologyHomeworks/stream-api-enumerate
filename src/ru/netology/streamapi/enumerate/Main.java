package ru.netology.streamapi.enumerate;
...
public class Main {
    public static void main(String[] args) {
        ...
        Collection<Person> familyList = persons.stream()
                .filter(p -> p.getEducation().equals(Education.HIGHER))
                .filter(p -> p.getAge() >= 18 &&
                        (p.getAge() <= 60 && p.getSex().equals(Sex.WOMAN) || p.getAge() <= 65 && p.getSex().equals(Sex.MAN)))
